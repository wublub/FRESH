...
FRESH 是一个 Windows 桌面便签工具，集**文字便签 + 截图归档 + 附件追踪**于一体。所有数据加密存储于本地，支持多账户与密码保护，并对外部附件的移动、改名做了深度追踪，让"被你扔到某个角落的那份文件"也能继续被打开。
...
- **截图工作区** — 框选截图、拖入图片、粘贴剪贴板图片、查看大图、保存为文件。
- **归档分类** — 给截图和备忘录添加分类，在归档列表和时间线中筛选查看。
...
首次启动后，FRESH 会自动在 `%APPDATA%\FRESH` 生成两个可编辑文件：`ui_text.json` 和 `custom.qss`。
...
截图工作区的「＋ 添加截图」进入框选截图；**更多 → 选择图片文件...** 可从磁盘添加已有图片。
...
| 类型 | 行为 | 适用 |
| --- | --- | --- |
| **复制型** | 复制到 `%APPDATA%\FRESH\accounts\<id>\attachments`（加密） | 截图、剪贴板图片、小文件归档 |
